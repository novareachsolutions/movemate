// src/modules/buyfromstore/buyfromstore.service.ts
...
export class BuyFromStoreService {
     ...
     */
    async createOrder(data: TBuyFromStoreOrder, customerId: number): Promise<BuyFromStoreOrder> {
        logger.debug('BuyFromStoreService.createOrder: Creating a new BuyFromStore order');
        const queryRunner = dbRepo(BuyFromStoreOrder).manager.connection.createQueryRunner();
            ...
            if (data.items && data.items.length > 0) {
                for (const itemData of data.items) {
                    const item = queryRunner.manager.create(Item, {
                        ...
                        currentStatus: OrderStatusEnum.PENDING, // Assuming default status
                    });
     ...
     */
    async cancelOrder(orderId: number, reason: string, userId: number): Promise<BuyFromStoreOrder> {
     ...
     */
    async leaveReview(orderId: number, rating: number, comment: string, userId: number): Promise<any> { // Replace `any` with your Review entity
        ...
        // Assuming you have an OrderReview entity similar to Report
     ...
     */
    async getOrderDetails(orderId: number, userId: number): Promise<BuyFromStoreOrder> {
     ...
     */
    async getAllOrders(userId: number, query: any): Promise<BuyFromStoreOrder[]> {
     ...
     */
    async createReport(orderId: number, reason: string, details: string, userId: number): Promise<Report> {
        logger.debug(`BuyFromStoreService.createReport: Creating report for order ID ${orderId}`);

        if (!reason) {
            throw new BuyFromStoreReportReasonRequiredError('Report reason is required');
        }
        ...
        const report = dbRepo(Report).create({
            reason,
            details,
            buyFromStoreOrder: order,
            customer: order.customer,
        ...
        const savedReport = await dbRepo(Report).save(report);
        logger.debug(`BuyFromStoreService.createReport: Report created with ID ${savedReport.id} for order ID ${orderId}`);
